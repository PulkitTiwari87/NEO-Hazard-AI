  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <Stat label="Random seed" value={String(data.random_seed)} />
        <Stat label="CV folds" value={String(data.n_cv_folds)} />
        <Stat label="Outer test size" value={`${data.outer_test_size * 100}%`} />
        <Stat label="Feature sets" value={String(data.feature_sets.length)} />
      </div>
      ...
      {data.last_benchmark_run && (
        <div className="rounded-lg border border-white/10 bg-white/[0.02] p-4 text-xs text-slate-300">
