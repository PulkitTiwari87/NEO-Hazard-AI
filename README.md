...
**Primary results:** [`docs/RESULTS.md`](docs/RESULTS.md) — the full
Experiment A-D comparison, generated from a real benchmark run
(`results/experiments/**`, 500-object NeoWs sample, seed 42, completed
2026-09-18) and copied verbatim from that run's deploy log (that
directory is gitignored pipeline output, same as `data/`; see
`docs/REPRODUCIBILITY.md`).

**Headline finding:** removing `moid_au` and `absolute_magnitude_h` (the
two fields NASA/JPL's PHA rule directly thresholds) drops the best
model's test F1 from **1.000 (Experiment A)** to **0.745 (Experiment B,
the primary leakage-aware result)** — a real, substantial drop that
confirms the leakage hypothesis in
[`docs/FEATURE_AUDIT.md`](docs/FEATURE_AUDIT.md). It does **not** drop to
the `DummyClassifier` baseline (F1 = 0.000, ROC-AUC = 0.500): every
leakage-aware model clears ROC-AUC 0.84–0.94, meaning NEO orbital and
physical/kinematic characteristics carry real, held-out-verified signal
about the classification on their own — just far less cleanly than the
two fields that define it. See
[`docs/RESULTS.md`](docs/RESULTS.md#interpretation) for the full
per-model table and interpretation.

| Experiment | Best model | Test F1 | Test ROC-AUC |
|---|---|---|---|
| A — Original (includes `moid_au`, `absolute_magnitude_h`) | xgboost | 1.000 | 1.000 |
| B — Leakage-Aware (primary result) | xgboost | 0.745 | 0.934 |
| C — Physical/Kinematic-only | random_forest | 0.719 | 0.894 |
| D — Orbital-only | xgboost | 0.711 | 0.882 |
| (baseline, every experiment) | `DummyClassifier` | 0.000 | 0.500 |

The Experiment A near-perfect scores are expected, not impressive: NASA's
...
inputs in that experiment only. Anomaly detection (a separate, unrelated
unsupervised pass) flagged 63/500 (12.6%) objects in this sample as
statistical outliers.

Re-running `python -m src.experiments.run_all` against a different or
larger ingestion will produce different real numbers — none of these are
fixed benchmarks, they describe one specific dataset snapshot.
